...
function NoteNodeComponent({ id, data }: NodeProps) {
  ...
  const colors: Record<string, { bg: string; border: string; header: string }> = {
    yellow: {
      ...
      header: "bg-yellow-200 dark:bg-yellow-800",
    },
    blue: {
    ...
    pink: {
      ...
      header: "bg-pink-200 dark:bg-pink-800",
    },
  };
  ...
  return (
    ...
    >
      <div
        className={`flex cursor-pointer items-center justify-between rounded-t-lg ${c.header} px-3 py-1`}
        onClick={cycleColor}
      >
      ...
      <div className="w-full min-w-[200px] p-2 nopan nodrag nowheel">
        ...
        <Textarea
          ...
          rows={4}
          className="nodrag nopan nowheel min-h-[5rem] w-full border-none bg-transparent text-xs focus-visible:ring-0"
        />
      </div>
    </div>
  );
